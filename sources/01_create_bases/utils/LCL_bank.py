...
class LCL_GENERATOR_CLASS():
    ...
    def __init__(self, 
        ...
        super(LCL_GENERATOR_CLASS, self).__init__()
        self._bank                  = bank
    ...
    def get_data(self):
        ...
        """
        LCL_COMBINED_DA5TA = get_fakedataset(bank='LCL')
        random.shuffle(LCL_COMBINED_DA5TA)
        return LCL_COMBINED_DA5TA
        ...
        for row_index, row_y in enumerate(range(self._TEMPLATE_OPTIONS['Y_MIN'] , self._TEMPLATE_OPTIONS['Y_MAX']  + 1, self._TEMPLATE_OPTIONS['Y_GAP']), start=1):
            for c_column in range(1,6):
                    ...
                    if stop_multi_row or row_index == row_index_precedent:   
                        ...
                        if c_column == 1: # Ajouter la transaction à la liste d'aconnotation seulement lorsqu'on est à la première colonnes. Sinon risque de doublons
                        ...
                        if c_column == 1:
                            ...
                            ANNOT_ARGS['dates'].append(data.replace('.','/'))
                        elif c_column == 4:
                    ...
                    if c_column == 1 or c_column == 3:
                        ...
                        text_x = cols_coords[0] + box_width - draw.textsize(data, FF_REGULAR)[0]
                    
                    if row_index <= self._TEMPLATE_OPTIONS['MAX_LEN_TRANSAC']:
                        # TODO : pprint(FF_REGULAR.getbbox(data,))
    ...
    def generate_many(self, n=10):
        for i in track(range(1, n+1), description='Processing...'):
            self.generate_one_image(img_name='lcl_image_{}'.format(i))
